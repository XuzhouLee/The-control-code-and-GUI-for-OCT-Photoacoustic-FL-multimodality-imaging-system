...
function updateChanneloption_Callback(src, event, handles)
    ...
    if (channeloptionnum == 3 && samplingRate > 500) %make sure that DAQ card will not be out of memory
    ...
    if channeloptionnum == 1
        setappdata(0, 'ActivChNum', 1);
        setappdata(0, 'Channeloption',7);
    elseif channeloptionnum == 2
        setappdata(0, 'ActivChNum', 2);
        setappdata(0, 'Channeloption',3);
    elseif channeloptionnum == 3
