...
setappdata(0, 'PreTrigSampl', 16*10); %number of pretrigger samples per channel to capture. Multiple of 16; Set to 0 for delay mode
...
function updatePreTrigSampl_Callback(src, event, handles)
    ...
    pretrig = uint16(str2num(pretrig));
    if (pretrig < 0 || pretrig > 3200)
        errordlg('Invalid pretigger sample number. Must be a multiple of 16 less than 3200');
        set(src, 'String', num2str(getappdata(0, 'PreTrigSampl')));
    elseif rem(pretrig, 16) ~= 0
        errordlg('Pretrigger rounded to the nearest multiple of 16.') 
        pretrig = pretrig/16*16;
