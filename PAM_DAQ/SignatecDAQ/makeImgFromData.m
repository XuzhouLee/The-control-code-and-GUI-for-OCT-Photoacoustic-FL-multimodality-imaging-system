...
SegmentSampleNum = acq.AcqLengthPoint*acq.ActivChNum;
...
for i=1:iRows
    ...
    if acq.ActivChNum == 1
        CHN3 = Segment(1:acq.ActivChNum:end,:,:);
    elseif acq.ActivChNum == 2
...
if acq.ActivChNum ~= 1
   ...
   figure(121); imagesc(uint8(CH1max)); colormap gray, axis equal
%    set(gca, 'xtick', [], 'ytick', [])
end

figure(123); imagesc(uint8(imrotate(flipud(CH3max),-90,'crop'))); color bar, axis equal
% set(gca, 'xtick', [], 'ytick', [])
...
if acq.ActivChNum == 4
    ...
    figure(122); imagesc(uint8(CH2max)); colormap gray, axis equal
%     set(gca, 'xtick', [], 'ytick', [])
    figure(124); imagesc(uint8(CH4max)); colormap gray, axis equal
%     set(gca, 'xtick', [], 'ytick', [])
