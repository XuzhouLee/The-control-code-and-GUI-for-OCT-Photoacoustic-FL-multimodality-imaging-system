...
while getappdata(0, 'isRunning') == 1 
    while (handles.tcpipClient.BytesAvailable > 0)
        ...
        tStamp = clock;
        name_suffix = [num2str(tStamp(1)), num2str(tStamp(2), '%02d'), num2str(tStamp(3), '%02d'), '-', num2str(tStamp(4),'%02d'),num2str(tStamp(5),'%02d'),num2str(fix(tStamp(6)),'%02d')];
...
%                 disp('trigger armed')
        ...
        elseif patternType == 3
            ...
            MemNeeded = params(2)*(params(1)+params(3))*getappdata(0, 'ActivChNum')*getappdata(0, 'AcqLengthPoint');
            if MemNeeded > 2e9
...
%     set(handles.hAcqStatsFld, 'String', 'Waiting for trigger');
    ...
    set(handles.hRcrLenFld,'string',getappdata(0, 'AcqLengthPoint'));
